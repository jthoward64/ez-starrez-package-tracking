...
// ==/UserScript==
{
    ...
    function getEntryId() {
        var _a;
        const hash = (_a = window.top) === null || _a === void 0 ? void 0 : _a.location.hash;
        const indexOfHash = hash === null || hash === void 0 ? void 0 : hash.indexOf("entry");
        if (indexOfHash == null) {
            return null;
        }
        const hashStartingWithEntry = hash === null || hash === void 0 ? void 0 : hash.substring(indexOfHash);
    ...
    function createSelect(statusId, trackingNumberId, selectTitle, selectSlug, formName, selectOptions, defaultValueCookieName) {
        let defaultValue = "";
        ...
        if (defaultValue)
            dropdownCaption.innerText = defaultValue;
        ...
        if (defaultValueCookieName) {
            ...
            lockButton.onclick = () => {
                const select = document.getElementById(`${statusId}-${selectSlug}-${trackingNumberId}`);
                if (select != null) {
                    if (isValueSaved) {
    ...
    function makeStarRezHeaders() {
        ...
        myHeaders.append("__RequestVerificationToken", 
        // @ts-ignore
    ...
    function getPossibleBuildings() {
        const xhr = new XMLHttpRequest();
        xhr.open("GET", `/StarRezWeb/Main/EntryCustomField/EditKeyData?parentID=${getEntryId()}&key=CustomField_Package%20Tracking`);
            ...
            xhr.onload = () => {
                if (xhr.readyState === xhr.DONE && xhr.status === 200) {
                    ...
                    if (selectElement == null) {
                        ...
                        reject("Select element is not the last one, the EZ Package Tracker is probably broken");
                        return;
                    }
                    else {
                        const buildings = [];
     ...
     */
    function submitPackage({ status, location, shippingType, trackingNumber, building, comments, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const res = yield fetch("https://starport.uky.edu/StarRezWeb/Main/EntryCustomField/EditKeyData", {
                method: "POST",
                headers: myHeaders,
            ...
            const secondRes = yield fetch(`https://starport.uky.edu/StarRezWeb/Main/EntryCustomField/ShowKey?key=CustomField_Package%20Tracking&parentID=${entryId}`, {
                method: "GET",
                headers: myHeaders,
                redirect: "follow",
                credentials: "include",
            ...
            if (secondRes.status !== 200) {
                console.error(yield secondRes.text());
                throw new Error("Failed to submit package, see log for more details");
            }
            alert("Package submitted successfully!");
            console.log("Package submitted successfully!", yield res.json(), yield secondRes.json());
        });
    }
    /**
     * Shows the form to submit a package
     */
    function handleTrackingNumberSubmit(trackingNumber, status) {
        return __awaiter(this, void 0, void 0, function* () {
            let statusId;
            ...
            const onPackageSelectionChanged = (selector) => {
                ...
                if (selector instanceof HTMLSelectElement && selectorText != null) {
                    selectorText.innerText = (_b = (_a = selector.selectedOptions.item(0)) === null || _a === void 0 ? void 0 : _a.label) !== null && _b !== void 0 ? _b : "";
                }
            ...
            if (mainPane) {
                ...
                if (shippingTypeSelect == null ||
                    ...
                    if (trackingNumber.startsWith("TBA") ||
                        ...
                        trackingNumber.match(/\b((98\d\d\d\d\d?\d\d\d\d|98\d\d) ?\d\d\d\d ?\d\d\d\d( ?\d\d\d)?)\b/) ||
    ...
    function onEntryPageOpened() {
        ...
        function keyDownReceivedPackage(event) {
            if (event.key === "Enter") {
        ...
        function keyDownIssuedPackage(event) {
            if (event.key === "Enter") {
                if (event.target == null ||
                    !(event.target instanceof HTMLInputElement)) {
                    throw new Error("event.target is not an input element");
